...
export function RentACarBuilder({ 
  entries, 
  onChange, 
  inputMethod, 
  overallMargin, 
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-semibold flex items-center gap-2">
                ...
                {t('builders.carEntry', { number: index + 1 })}
              </h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                <Label>{t('builders.carTypeRequired')}</Label>
                <Select
                  value={entry.carType}
                  onValueChange={(val) => updateEntry(index, "carType", val)}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t('builders.selectCarType')} />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                <Label>{t('builders.pickupLocationRequired')}</Label>
                <Input
                  placeholder={t('builders.pickupLocationPlaceholder')}
              ...
              <div className="space-y-2">
                <Label>{t('builders.dropoffLocationRequired')}</Label>
                <Input
                  placeholder={t('builders.dropoffLocationPlaceholder')}
              ...
              <div className="space-y-2">
                <Label className="text-orange-700">{t('builders.priceAfterMarginCommission', { percent: (parseFloat(overallMargin) || 0) + (parseFloat(commission) || 0) })}</Label>
                <Input
                  value={`${proposalCurrency} ${((calculateTotal(entry) || 0) * (1 + ((parseFloat(overallMargin) || 0) + (parseFloat(commission) || 0)) / 100)).toFixed(2)}`}
                  disabled
                  className="bg-orange-50 font-bold text-orange-700 border-orange-300"
                />
              </div>
            </div>
          </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-orange-50 border-b">
            <tr>
              ...
              <th className="p-2 text-left min-w-[120px]">{t('builders.tableHeaderCarType')}</th>
              <th className="p-2 text-left min-w-[150px]">{t('builders.tableHeaderPickup')}</th>
              <th className="p-2 text-left min-w-[150px]">{t('builders.tableHeaderDropoff')}</th>
